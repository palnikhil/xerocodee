...
authController.GoogleAuth = async(req) => {
  ...
  if(users.data.length > 0){
      if(users.data[0].provider === "EMAIL"){
        return appHelper.apiResponse(401, false, 'Sign In with Email and Password');
      }
      ...
      return appHelper.apiResponse(201, true, 'Auth Success',users.data[0]);
  }
  else{
      let provider = 'GOOGLE';
      let item =  {
...
authController.login = async (req) => {
  try{
    ...
    return appHelper.apiResponse(201, true, 'Auth Success',userData);
  }catch(err){
    console.log( err.message ? err.message : err);
    return appHelper.apiResponse( 500, false,err.message ? err.message : err );
  }
